...
trait CompoundFigure {
  ...
  private def seqMatchRules(seq: Seq[Square]): Boolean = {
    seq.size match {
      case (2) => seq.head.rightUp + seq(1).leftUp <= maxSum && seq.head.rightDown + seq(1).leftDown <= maxSum
      case (4) => seq.head.rightDown + seq(1).leftDown + seq(2).rightUp + seq(3).leftUp == maxSum &&
      ...
      case (12) => seq.head.leftDown + seq(2).rightUp + seq(3).leftUp <= maxSum &&
